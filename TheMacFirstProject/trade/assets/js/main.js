...
var AlicmsWebSocket = {
    ...
    AlicmsWebSocket.init = function () {
       
        // if(top.location.pathname.indexOf('/Ceshi') == -1 && top.location.pathname.indexOf('/Ceshi') == -1){
    ...
    AlicmsWebSocket.unBinary = function (datas, callback) {
    ...
    AlicmsWebSocket.sendMessage = function () {
    ...
    AlicmsWebSocket.dealMessage = function (json) {
        ...
        if(result.event =="chat"){
            return false;
        }
         if(result.event =="usercount"){
            return false;
        }
        ...
        if(channel.indexOf("bargain") != -1){
            ...
            for (var i = 0; i < result.data.length; i++) {
                ...
                if(result.data[i].type==1){
                    ...
                    result.data[i].type="buy";

                }else{

                    result.data[i].type="sell";

                }

            }
            
            console.log(_this['dealRecord'].method,_this['dealRecord'].method(result))
    ...
    AlicmsWebSocket.onOpen = function (event) {
    ...
    AlicmsWebSocket.onMessage = function (event) {
